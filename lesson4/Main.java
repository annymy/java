...
public class Main {
    ...
    public static boolean isCheckWin(char c) {
        ...
        for (int i = 0; i < map.length; i++) {
            ...
            //не хватило времени приумать, как циклом организовать проверку по маленьким диагоналям
        if(map[0][3] == c && map[1][2] == c&& map[2][1] == c && map[3][0] == c) return true;
        if(map[1][4] == c && map[2][3] == c && map[3][2] == c  && map[3][1] ==c) return true;
        if(map[0][1] == c && map[1][2] == c && map[2][3] == c && map[3][4] == c)return true;
        if(map[1][0] == c && map[2][1] == c && map[3][2] == c && map[4][3] == c) return true;
